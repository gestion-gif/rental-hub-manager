...
export function PropertyPicker({
  ...
  const options: Item[] = [{ id: "all", name: allLabel }, ...items];
  ...
  return (
    <>
      <Pressable testID={testID} onPress={() => setOpen(true)} style={styles.trigger}>
        ...
        <Text style={styles.triggerText} numberOfLines={1}>{current?.name}</Text>
      ...
      <Modal visible={open} transparent animationType="fade" onRequestClose={() => setOpen(false)}>
        <Pressable style={styles.backdrop} onPress={() => setOpen(false)}>
          <Pressable style={[styles.sheet, { paddingBottom: insets.bottom + spacing.md }]} onPress={() => {}}>
            ...
            <Text style={styles.sheetTitle}>Choisir un logement</Text>
...
const styles = StyleSheet.create({
  ...
  triggerText: { flex: 1, fontFamily: font.semibold, fontSize: fontSize.base, color: colors.onSurface },
