...
export default function ThreadDetail() {
  ...
  async function suggest() {
    const lastGuest = [...messages].reverse().find((m: any) => !m.mine);
    if (!lastGuest) { setError("Aucun message voyageur à qui répondre."); return; }
    setAiLoading(true); setError(null);
    try {
      const r = await api.post("/ai/guest-reply", { guest_message: lastGuest.text, tone: "chaleureux" });
      setReply(r.reply);
    } catch { setError("Impossible de générer une proposition."); }
    setAiLoading(false);
  }
            ...
            <Pressable testID="ai-suggest" onPress={suggest} style={styles.suggestBtn} disabled={aiLoading}>
              {aiLoading ? (
                ...
                <>
                  ...
                  <Text style={styles.suggestText}>Proposer une réponse (IA)</Text>
                </>
              )}
            </Pressable>
