      ...
      {loading ? (
        ...
        >
          ...
          <View style={styles.sectionHead}>
            ...
            <View style={styles.count}><Text style={styles.countTxt}>{data?.departures?.length || 0}</Text></View>
          </View>
            ...
            data.departures.map((d: any) => (
              <View key={d.id} style={styles.card} testID={`departure-${d.id}`}>
                <View style={styles.cardRow}>
                  ...
                  {!!d.checkout_time && (
                    <View style={styles.timeTag}><Text style={styles.timeTxt}>{d.checkout_time}</Text></View>
                  )}
                </View>
          ...
          <View style={[styles.sectionHead, { marginTop: spacing.xl }]}>
            <Ionicons name="sparkles-outline" size={18} color={colors.onSurface} />
            <Text style={styles.sectionTitle}>Ménages à faire</Text>
            <View style={styles.count}><Text style={styles.countTxt}>{data?.cleanings?.length || 0}</Text></View>
          </View>
          {data?.cleanings?.length ? (
            data.cleanings.map((c: any) => (
              <Pressable
                key={c.id}
                testID={`cleaning-${c.id}`}
                onPress={() => router.push(`/intervention-form?id=${c.id}`)}
                style={[styles.card, c.done && styles.cardDone]}
              >
                <View style={styles.cardRow}>
                  <Ionicons
                    name={c.done ? "checkmark-circle" : "ellipse-outline"}
                    size={20}
                    color={c.done ? colors.success : colors.onSurfaceTertiary}
                  />
                  <Text style={[styles.prop, c.done && styles.propDone]} numberOfLines={1}>{c.property_name}</Text>
                  <Ionicons name="chevron-forward" size={16} color={colors.onSurfaceTertiary} />
                </View>
                <Text style={styles.sub}>
                  {c.done ? "Terminé" : "À faire"}{c.intervenant ? ` · ${c.intervenant}` : ""}
                </Text>
              </Pressable>
            ))
          ) : (
